...
class Day02(private val input: List<List<Int>>) {
    ...
    private fun isSafe(levels: List<Int>): Int {
        ...
        levels.forEachIndexed { index, current ->
            ...
            if (current != previous + increasing * 1 && current != previous + increasing * 2 && current != previous + increasing * 3) {
                return index
            }
            previous = current
        }
    ...
    private fun isSafeWithRemoval(levels: List<Int>): Boolean {
        ...
        return when (isSafe) {
            0 -> true
            else -> isSafe(levels.filterIndexed { index, _ -> index != isSafe - 1 }) == 0 || isSafe(levels.filterIndexed { index, _ -> index != isSafe }) == 0
        }
    }
}
